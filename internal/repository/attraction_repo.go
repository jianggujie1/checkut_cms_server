...
const attractionCols = `id, destination_id, title, subtitle, image_url, duration, tag, status, created_at, updated_at, deleted_at`

func scanAttraction(row pgx.Row) (*model.Attraction, error) {
	var a model.Attraction
	err := row.Scan(
		&a.ID, &a.DestinationID, &a.Title, &a.Subtitle, &a.ImageURL, &a.Duration, &a.Tag,
...
func (r *AttractionRepo) Create(ctx context.Context, a *model.Attraction) (*model.Attraction, error) {
	...
	err := r.db.QueryRow(ctx,
		`insert into attractions (destination_id, title, subtitle, image_url, duration, tag, status)
		 values ($1,$2,$3,$4,$5,$6,$7) returning `+attractionCols,
		a.DestinationID, a.Title, a.Subtitle, a.ImageURL, a.Duration, a.Tag, a.Status,
	).Scan(
		&created.ID, &created.DestinationID, &created.Title, &created.Subtitle, &created.ImageURL,
		&created.Duration, &created.Tag, &created.Status, &created.CreatedAt, &created.UpdatedAt, &created.DeletedAt,
	)
	if err != nil {
		return nil, err
	}
...
func (r *AttractionRepo) Update(ctx context.Context, a *model.Attraction) (*model.Attraction, error) {
	...
	err := r.db.QueryRow(ctx,
		`update attractions set destination_id=$1, title=$2, subtitle=$3, image_url=$4, duration=$5, tag=$6
		 where id=$7 and deleted_at is null returning `+attractionCols,
		a.DestinationID, a.Title, a.Subtitle, a.ImageURL, a.Duration, a.Tag, a.ID,
	).Scan(
		&updated.ID, &updated.DestinationID, &updated.Title, &updated.Subtitle, &updated.ImageURL,
		&updated.Duration, &updated.Tag, &updated.Status, &updated.CreatedAt, &updated.UpdatedAt, &updated.DeletedAt,
	)
	if errors.Is(err, pgx.ErrNoRows) {
		return nil, ErrNotFound
	}
